...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Rhino Miner Coin address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no rhino: URI
    if(!uri.isValid() || uri.scheme() != QString("rhino"))
        return false;
...
{
    QString ret = QString("rhino:%1").arg(info.address);
    int paramCount = 0;
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Rhino.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Rhino (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Rhino (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "rhino.desktop";
    return GetAutostartDir() / strprintf("rhino-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a rhino.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Rhino Miner Coin\n";
        else
            optionFile << strprintf("Name=Rhino Miner Coin (%s)\n", chain);

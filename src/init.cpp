 ...
 */
static const char* BITCOIN_PID_FILENAME = "rhino.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/rhinominer/rhino-miner-coin>";
    const std::string URL_WEBSITE = "<https://rhinominer.rocks>";
    ...
    std::string yetenCopyright = strprintf(_("Copyright (C) %i-%i"), 2017, 2025) + " The Yeten Core Team";
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if Rhino Miner Coin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Rhino Miner Coin is started while in a temporary directory.\n",

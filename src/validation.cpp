...
#if defined(NDEBUG)
# error "Rhino Miner Coin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Rhino Miner Coin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s RMC additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    const CAmount tail_subsidy = COIN / 10;
    // Force block reward to zero when right shift is undefined.
    if (halvings >= 64)
        return tail_subsidy; // Tail emission after the primary schedule completes.

    CAmount nSubsidy = 50 * COIN;
    // Subsidy is cut in half every nSubsidyHalvingInterval blocks.
    // Tail emission is applied if the halved subsidy drops below 0.1 RMC.
    nSubsidy >>= halvings;
...
//if (pindex->nHeight < 1988319) {
//LogPrintf("old Height<1988319 \n");
//}
